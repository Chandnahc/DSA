...
You are also given an integer array `weights` of length `26`, where `weights[i]` represents the weight of the $i^{th}$ lowercase English letter (i.e., `weights[0]` for `'a'`, `weights[1]` for `'b'`, etc.).
...
**Output:** `"yyy"`  
**Explanation:** * Each word has weight 1. The result modulo 26 is $1 \pmod{26} = 1$, which maps to `'y'`.  
...
**Output:** `"g"`  
**Explanation:** * The weight of `"abcd"` is $7 + 5 + 3 + 4 = 19$. The result modulo 26 is $19 \pmod{26} = 19$, which maps to `'g'`.  
